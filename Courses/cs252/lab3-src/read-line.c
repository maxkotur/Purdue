...
char * read_line() {
  ...
  line_length = 0;
  line_loc = 0;
  memset(&line_buffer[0], 0, sizeof(line_buffer));
    ...
    if (ch>=32) {
      ...
      if (ch == 127) {
        if (line_length > 0) {
				  ...
				  line_length--;
				  line_loc--;
          removeChar(line_buffer, line_loc);
			  }
			  continue;
      }
      // Do echo
      ...
      if (ch1==91 && ch2==65) {
        ...
        int i = 0;
	     ...
	     for (i =0; i < line_length; i++) {
	      ...
	      // Copy line from history
	      if (history_length > 0 && history_index < history_length && history_index >= 0) {
          strcpy(line_buffer, history[history_index--]);
          if (history_index == -1) {
            history_index = 0;
          }
