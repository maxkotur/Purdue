...
public class ProductPlan2 implements Plan {
    ...
    */
   public ProductPlan2(Plan p1, Plan p2, List<Term> terms) {
      this.p1 = p1;
      this.p2 = p2;
      for (Term t : terms) {
         if (p1.schema().hasField(t.getLhs().toString())){
            this.fieldname1 = t.getLhs().toString();
         }
         if (p1.schema().hasField(t.getRhs().toString())){
            this.fieldname1 = t.getRhs().toString();
         }
         if (p2.schema().hasField(t.getLhs().toString())){
            this.fieldname2 = t.getLhs().toString();
         }
         if (p2.schema().hasField(t.getRhs().toString())){
            this.fieldname2 = t.getRhs().toString();
         }
   ...
   public Scan open() {
      Scan s1 = p1.open();
      Scan s2 = p2.open();
      return new ProductScan2(s1, s2, fieldname1, fieldname2);
   }
